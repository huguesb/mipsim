...
void elf_file_load(ELF_File *elf, const char *filename)
{
    if ( elf == NULL )
    {
    ...
    *elf->header = hdr;
    ...
    if ( fseek(handle, hdr.e_phoff, SEEK_SET) )
    {
    ...
    if ( hdr.e_phnum )
        elf->segments = (ELF_Segment**)malloc(hdr.e_phnum * sizeof(ELF_Segment*));
    
    if ( elf->segments )
    {
        elf->nsegment = hdr.e_phnum;
        
        for ( ELF32_Word i = 0; i < hdr.e_phnum; ++i )
        {
            ELF32_Word soff = hdr.e_phoff + hdr.e_phentsize * i;
            
            if ( soff & 3 )
                printf("Suspicious (un)alignement\n");
            ...
            if ( fseek(handle, soff, SEEK_SET) )
            {
                ...
                if ( s->p_type == PT_LOAD )
                {
                    ...
                    if ( s->p_offset < hdr.e_phoff + hdr.e_phentsize * (hdr.e_phnum + 1) )
                    {
                    ...
                    {
                        printf("ELF:%s: Invalid file : unable to reach segment %d\n", filename, i);
                    } else {
                        ...
                        {
                            printf("ELF:%s: Invalid file : unable to read data of segment %d [%d vs %u]\n", filename, i, sz, s->p_filesz);
                            free(s->p_data);
                            s->p_data = NULL;
                        } else {
                            printf("ELF:%s: Succesfully loaded segment %d (%d bytes)\n", filename, i, s->p_filesz);
                        }
                    }
                }
            }
